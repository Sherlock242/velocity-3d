...
export function createOpenCollegeBuilding() {
  ...
  function createWing(width: number, depth: number) {
      ...
      for (let i = 0; i < numFloors; i++) {
          ...
          const mainWall = new THREE.Mesh(wallGeom, whiteMaterial);
          mainWall.position.y = floorHeight / 2;
          floorGroup.add(mainWall);

          // Details for the front-facing (courtyard) side
          const frontBorder = new THREE.Mesh(new THREE.BoxGeometry(width, 1, 1.2), greenMaterial);
          ...
          for (let j = 0; j < numPillars; j++) {
              ...
              const pillar = new THREE.Mesh(pillarGeom, greenMaterial);
          ...
          const backBorder = new THREE.Mesh(new THREE.BoxGeometry(width, 1, 1.2), greenMaterial);
